...
const achievements = [
  { icon: "cafe" as const, title: "First sip", detail: "Saved your first café" },
  { icon: "sparkles" as const, title: "Small adventure", detail: "Finished a SIDEQUEST" },
  { icon: "leaf" as const, title: "Quiet finder", detail: "Found three calm spots" },
];
  ...
  return (
    <Screen>
      <View style={styles.toolbar}>
        <Text style={styles.kicker}>CAFÉ PASSPORT</Text>
      ...
      <View style={styles.passport}>
        ...
        <Text style={styles.handle}>LEVEL {level} · MAKATI EXPLORER</Text>
        <View style={styles.progress}><View style={[styles.progressFill, { width: `${(levelProgress / 250) * 100}%` }]} /></View>
        <Text style={styles.progressText}>{250 - levelProgress} XP to the next stamp</Text>
      ...
      <View style={styles.sectionHeading}>
        ...
        <Text style={styles.sectionCount}>{achievements.length} earned</Text>
      </View>
      ...
      <Pressable onPress={onOpenSocial} style={styles.actionRow}>
        ...
        <View style={styles.actionCopy}><Text style={styles.actionTitle}>Friend activity</Text><Text style={styles.actionDetail}>See the places your circle tried.</Text></View>
        <Ionicons name="chevron-forward" size={19} color={colors.inkMuted} />
      </Pressable>
      <Pressable onPress={onOpenPlus} style={styles.plusCard}>
        <View><Text style={styles.plusKicker}>SIDEQUEST+</Text><Text style={styles.plusTitle}>More routes, never more pressure.</Text><Text style={styles.plusLink}>See SIDEQUEST+</Text></View>
...
const styles = StyleSheet.create({
  ...
  passport: { alignItems: "center", marginTop: 18, padding: 20, borderWidth: 1, borderColor: colors.border, borderRadius: radii.xl, backgroundColor: colors.paper },
  avatarRing: { width: 106, height: 106, alignItems: "center", justifyContent: "center", borderWidth: 2, borderStyle: "dashed", borderColor: colors.caramel, borderRadius: 53 },
  avatar: { width: 82, height: 82, alignItems: "center", justifyContent: "center", borderRadius: 41, backgroundColor: colors.espresso },
  avatarText: { color: colors.crema, fontFamily: typography.display, fontSize: 26 },
  name: { marginTop: 14, color: colors.espresso, fontFamily: typography.display, fontSize: 30 },
  ...
  stamps: { flexDirection: "row", gap: 8 },
  stamp: { minHeight: 126, flex: 1, padding: 11, borderWidth: 1, borderStyle: "dashed", borderColor: colors.caramel, borderRadius: 40, backgroundColor: colors.caramelMist },
  stampSage: { borderColor: colors.sage, backgroundColor: colors.sageMist },
  stampTitle: { marginTop: 8, color: colors.espresso, fontFamily: typography.displaySemibold, fontSize: 13 },
  stampDetail: { marginTop: 3, color: colors.inkMuted, fontFamily: typography.body, fontSize: 8, lineHeight: 12 },
  ...
  plusCard: { minHeight: 132, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 12, padding: 18, borderRadius: radii.lg, backgroundColor: colors.espresso },
