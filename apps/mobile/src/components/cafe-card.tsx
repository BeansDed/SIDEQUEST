  ...
  return (
    <View style={[styles.card, compact && styles.compact]}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Open ${cafe.name}`} onPress={onOpen} style={[styles.photoWrap, compact && styles.compactPhoto]}>
        <Image source={cafe.image} accessibilityLabel={cafe.imageAlt} style={styles.photo} contentFit="cover" transition={state.settings.reducedMotion ? 0 : 180} />
        <View style={styles.scrim} />
        <View style={styles.matchPill}><Text style={styles.matchPillText}>{cafe.source === "google" ? `★ ${cafe.rating || "NEW"} · GOOGLE` : `${score}% match`}</Text></View>
      </Pressable>
      <View style={styles.copy}>
        <View style={styles.titleRow}>
          <Pressable accessibilityRole="button" accessibilityLabel={`View ${cafe.name} details`} onPress={onOpen} style={styles.titlePressable}><Text style={styles.kicker}>{cafe.neighborhood}</Text><Text style={[styles.title, compact && styles.compactTitle]}>{cafe.name}</Text></Pressable>
        ...
        <Text style={styles.vibes}>{cafe.source === "google" ? cafe.description : [...cafe.vibes, cafe.useCases[0]].join(" · ")}</Text>
        <View style={styles.meta}>
          <Text style={styles.metaText}>{cafe.walkMinutes} min walk</Text><Text style={styles.dot}>•</Text>
          <Text style={styles.metaText}>{cafe.source === "google" ? `₱${cafe.averagePrice} price guide` : `₱${cafe.averagePrice} avg`}</Text><Text style={styles.dot}>•</Text>
...
const createStyles = (colors: AppPalette) => StyleSheet.create({
  ...
  compactPhoto: { width: 100, height: "auto", minHeight: 132 },
  ...
  kicker: { color: colors.inkMuted, fontFamily: typography.bodyMedium, fontSize: 12 },
  title: { marginTop: 4, color: colors.espresso, fontFamily: typography.display, fontSize: 24, lineHeight: 29 },
  compactTitle: { fontFamily: typography.bodyBold, fontSize: 17, lineHeight: 22 },
